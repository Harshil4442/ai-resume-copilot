...
function SkillBadge({ skill }: { skill: string }) {
  return <span className="px-2 py-1 rounded-lg bg-gray-100 text-gray-700 text-xs">{skill}</span>;
}
...
function GapRow({ gap }: { gap: MarketResumeGap }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_120px_120px_1.6fr] gap-3 rounded-lg border border-gray-200 bg-white p-4">
      <div>
...
export default function MarketPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto py-10 space-y-8">
      <section className="rounded-lg bg-gray-950 text-white overflow-hidden border border-gray-900">
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="p-6 md:p-8">
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-blue-200">Market Skills Intelligence</div>
            <h1 className="text-3xl md:text-5xl font-black mt-4 max-w-3xl">
          ...
          <div className="bg-white text-gray-950 p-6 md:p-8 border-t lg:border-t-0 lg:border-l border-gray-200">
            <form onSubmit={analyze} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="w-full border rounded-lg px-3 py-2 text-sm uppercase" maxLength={2} value={countryCode} onChange={(e) => setCountryCode(e.target.value.toUpperCase())} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Experience</label>
                  <select className="w-full border rounded-lg px-3 py-2 text-sm" value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)}>
                ...
                <div>
                  ...
                  <select className="w-full border rounded-lg px-3 py-2 text-sm" value={remote} onChange={(e) => setRemote(e.target.value)}>
              ...
              <div>
                ...
                <select className="w-full border rounded-lg px-3 py-2 text-sm" value={resumeId} onChange={(e) => setResumeId(e.target.value)}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full border rounded-lg px-3 py-2 text-sm" type="number" min="5" max="100" value={maxResults} onChange={(e) => setMaxResults(Number(e.target.value))} />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wide text-gray-500 mb-1">Posted within</label>
                  <input className="w-full border rounded-lg px-3 py-2 text-sm" type="number" min="1" max="365" value={postedWithinDays} onChange={(e) => setPostedWithinDays(Number(e.target.value))} />
                </div>
              </div>
              <button disabled={loading || !targetRole.trim()} className="w-full rounded-lg bg-gray-950 text-white px-4 py-3 text-sm font-bold hover:bg-black disabled:opacity-50">
      ...
      {!data && !loading && (
        <section className="rounded-lg border border-gray-200 bg-white p-8 text-center">
          <h2 className="text-xl font-black text-gray-950">Ready for a live market snapshot</h2>
      ...
      {data && (
        <div className="space-y-8">
          <section className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-950 text-white p-5">
          ...
          <section>
            <div className="flex items-end justify-between gap-4 mb-4">
              <div>
                <h2 className="text-lg font-black text-gray-950">Resume vs market</h2>
            ...
            <div className="space-y-3">
              {data.resume_gap_analysis.length > 0 ? (
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-5 text-sm text-gray-500">No gap analysis available.</div>
              )}
            </div>
          </section>

          <section>
            <h2 className="text-lg font-black text-gray-950 mb-4">Project ideas to close market gaps</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {data.recommended_projects.map((project) => (
                <div key={project.title} className="rounded-lg border border-gray-200 bg-white p-5">
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-5">
            <h2 className="text-lg font-black text-gray-950 mb-4">Sample jobs used</h2>
