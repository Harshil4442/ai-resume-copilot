      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[450px_1fr] gap-6 lg:gap-8 items-stretch">
        ...
        <FadeIn>
          <GlassCard className="p-8 md:p-10 h-full flex flex-col justify-center bg-white/70 backdrop-blur-xl border-white" hoverEffect={false}>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Create Profile</div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter mb-2">Join the OS</h2>
            <p className="text-sm text-slate-500 font-medium mb-8">Create an account, upload a resume, and begin matching against the roles you want.</p>
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              <div className="space-y-4">
                <div>
                  ...
                  <input className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10 font-medium" placeholder="you@example.com" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-700 mb-1.5 px-1">Password</label>
                  <input className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10 font-medium" placeholder="Min 6 characters" type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={6} required />
                </div>
              </div>
            ...
            {ok && <div className="mt-6 text-sm text-emerald-700 font-bold bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-center">Account created successfully! <Link className="underline decoration-2" href="/login">Log in now</Link></div>}
          ...
          <StaggerContainer className="relative z-10 grid grid-cols-1 gap-4 mt-12">
            {[
              ...
              <StaggerItem key={item.title}>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm flex items-center gap-4 hover:bg-white/10 transition-colors">
